...
async function Login(req, res) {
    ...
    try {
        ...
        return res.status(401).json({ status: true, message: 'Login unsuccessful!', data: error })
    }

    return res.status(401).json({ status: true, message: 'Login unsuccessful!' })
}
...
function generateRefreshToken(user_id) {
    ...
    redis_client.get(user_id.toString(), (err, data) => {
        ...
        redis_client.set(user_id.toString(), JSON.stringify({
            token: refresh_token
        }))
    })
